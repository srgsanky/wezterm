...
wezterm.on("update-right-status", function(window, pane)
	if record_active_tab then
		record_active_tab(window, pane)
	end
...
record_active_tab = function(window, pane)
...
-- Tab switching
local tab_switcher_keys = {
	-- Visual fuzzy tab switcher, similar to IntelliJ/Chrome quick tab switchers.
	-- Entries are ordered by recency, exclude the current tab, and are prefixed with
	-- 1-based tab numbers so typing "12" jumps to tab 12.
	{
		key = "e",
		mods = "CMD",
